Sub WriteCampaignReport()
    ...
    For rowIndex = 3 To totalRowCount
        ...
        fConversion = Sheets(dataSheetName).Cells(rowIndex, 7)
    ...
    Call FillCellData(reportName, 24, 4, twoWeekDataArr)

    '向报表中写入前后对比的两天数据
    Call ComMetricArrayData(twoDayDataArr)
    Call FillCellData(reportName, 24, 9, twoDayDataArr)
...
'生成饼图
    ...
    arr = Sheets(dataSheetName).Range("C3:K" & n)
    
    For i = 1 To UBound(arr)
        If dicCampaign(arr(i, 1)) = "" Then
            dicCampaign(arr(i, 1)) = i
            rowCount = rowCount + 1
        End If
        dicMetricData(arr(i, 1) & "Imp") = dicMetricData(arr(i, 1) & "Imp") + arr(i, 2)
        dicMetricData(arr(i, 1) & "Click") = dicMetricData(arr(i, 1) & "Click") + arr(i, 3)
        dicMetricData(arr(i, 1) & "Cost") = dicMetricData(arr(i, 1) & "Cost") + arr(i, 4) * rate
        dicMetricData(arr(i, 1) & "Conversion") = dicMetricData(arr(i, 1) & "Conversion") + arr(i, 5)
        dicMetricData(arr(i, 1) & "Revenue") = dicMetricData(arr(i, 1) & "Revenue") + arr(i, 9) * rate
    Next
    ...
    If Not IsShapeExists(reportName, "campaignChart") Then
        Call Make3DPieChart(reportName, "AA2:AA" & (rowCount + 1) & ",AP2:AP" & (rowCount + 1), reportName, Range("H5").left, Range("H5").top, 450, 265, "campaignChart", "All Campaign", True)
    Else
